...
int Node::AddTrajectory(const TrajectoryOptions& options,
  ...
  const std::set<cartographer::mapping::TrajectoryBuilderInterface::SensorId>
      expected_sensor_ids = ComputeExpectedSensorIds(options, topics);//得到的是期望订阅主题的字符串列表
  const int trajectory_id =
      map_builder_bridge_.AddTrajectory(expected_sensor_ids, options);//增加Trajectory
  AddExtrapolator(trajectory_id, options); //推算位姿
  AddSensorSamplers(trajectory_id, options);
  LaunchSubscribers(options, topics, trajectory_id);//消息订阅，重要;进入匹配
